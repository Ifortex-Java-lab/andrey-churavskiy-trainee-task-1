...
public class StripePortalServiceImpl implements StripePortalService {
    ...
    public StripePortalResponse createPortalSession(StripePortalRequest request) {
        ...
        try {
            ...
            log.debug("Creating Stripe portal session for userId: {}, customerId: {}", user.getId(), user.getCustomerId());
            Session session = Session.create(params);

            log.info("Stripe portal session created for userId: {}. URL: {}", user.getId(), session.getUrl());
