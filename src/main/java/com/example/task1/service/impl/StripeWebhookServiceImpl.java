  ...
  public void handleEvent(StripeWebhookEventDto eventDto) {
    ...
    if (statusString != null) {
      try {
        ...
        log.warn(
            "Unknown subscription status '{}' for subscriptionId: {}",
            statusString,
    ...
    log.info(
        "Processing Stripe webhook event: {}, subscriptionId: {}",
        eventDto.getType(),
        stripeSubscriptionId);
    ...
    switch (eventDto.getType()) {
      case "customer.subscription.created":
        if (optionalSubscription.isEmpty()) {
          ...
          log.info(
              "Subscription with Stripe subscriptionId: {} already exists", stripeSubscriptionId);
        }
        break;
      ...
      case "customer.subscription.deleted":
        if (optionalSubscription.isPresent()) {
          ...
          log.warn(
              "Subscription not found for Stripe subscriptionId: {} for deletion",
              stripeSubscriptionId);
        }
        break;
